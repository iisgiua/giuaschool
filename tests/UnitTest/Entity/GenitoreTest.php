...
class GenitoreTest extends DatabaseTestCase {
  ...
  public function testAttributi() {
    ...
    for ($i = 0; $i < 3; $i++) {
      ...
      $email = $username.'.g@lovelace.edu.it';
      $alunno = $this->faker->randomElement($this->em->getRepository('App\Entity\Alunno')->findBy([], [], 20));
      foreach ($this->fields as $field) {
        $data[$i][$field] =
          $field == 'username' ? $username.'.g' :
          ($field == 'password' ? $this->encoder->encodePassword($o[$i], $username.'.g') :
          ($field == 'email' ? $email :
  ...
  public function testMetodi() {
    ...
    $this->assertTrue($existent instanceOf \App\Entity\Genitore, $this->entity.'instanceOf Genitore');
    ...
    $this->assertTrue(is_a($existent, 'App\Entity\Genitore'), $this->entity.'is_a Genitore');

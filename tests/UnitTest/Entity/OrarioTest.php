...
/**
 * Unit test dell'entità Orario
 *
 * @author Antonello Dessì
 */
class OrarioTest extends EntityTestCase {
  ...
  public function __construct() {
    ...
    // SQL read
    $this->canRead = ['gs_orario' => ['id', 'creato', 'modificato', 'nome', 'inizio', 'fine', 'sede_id']];
    // SQL write
    $this->canWrite = ['gs_orario' => ['id', 'creato', 'modificato', 'nome', 'inizio', 'fine', 'sede_id']];
    // SQL exec
  ...
  public function testProperties() {
    ...
    for ($i = 0; $i < 5; $i++) {
      ...
      sleep(1);
      $data[$i]['nome'] = $this->faker->passthrough(substr($this->faker->text(), 0, 64));
  ...
  public function testValidation() {
    ...
    // nome
    $property = $this->getPrivateProperty(Orario::class, 'nome');
    ...
    // inizio
    $property = $this->getPrivateProperty(Orario::class, 'inizio');
    ...
    // fine
    $property = $this->getPrivateProperty(Orario::class, 'fine');
    $property->setValue($existent, null);
    $err = $this->val->validate($existent);
    ...
    // sede
    $property = $this->getPrivateProperty(Orario::class, 'sede');
    $property->setValue($existent, null);
    $err = $this->val->validate($existent);

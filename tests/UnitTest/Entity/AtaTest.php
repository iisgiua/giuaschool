 ...
 */
class AtaTest extends DatabaseTestCase {
  ...
  public function __construct() {
    ...
    $this->fields = ['username', 'password', 'email', 'token', 'tokenCreato', 'prelogin', 'preloginCreato',
      ...
      'codiceFiscale', 'citta', 'indirizzo', 'numeriTelefono', 'notifica', 'tipo', 'segreteria', 'sede'];
    // fixture da caricare
  ...
  public function testAttributi() {
    ...
    for ($i = 0; $i < 3; $i++) {
      ...
      $email = $username.'.t@lovelace.edu.it';
      foreach ($this->fields as $field) {
        $data[$i][$field] =
          $field == 'username' ? $username.'.t' :
          ($field == 'password' ? $this->encoder->encodePassword($o[$i], $username.'.t') :
          ($field == 'email' ? $email :
          ...
          ($field == 'notifica' ? null :
          ($field == 'tipo' ? $this->faker->randomElement(['A', 'T', 'C', 'U', 'D']) :
          ($field == 'segreteria' ? $this->faker->randomElement([true, false]) :
  ...
  public function testMetodi() {
    ...
    $this->assertTrue($existent instanceOf \App\Entity\Ata, $this->entity.'instanceOf Ata');
    ...
    $this->assertTrue(is_a($existent, 'App\Entity\Ata'), $this->entity.'is_a Ata');

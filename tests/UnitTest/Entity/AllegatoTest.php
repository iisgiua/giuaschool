...
/**
 * Unit test dell'entità Allegato
 *
 * @author Antonello Dessì
 */
class AllegatoTest extends EntityTestCase {
 ...
 /**
  ...
  protected function setUp(): void {
    // nome dell'entità
    $this->entity = Allegato::class;
    // campi da testare
    $this->fields = ['titolo', 'nome', 'estensione', 'dimensione', 'file', 'comunicazione'];
    $this->noStoredFields = [];
    $this->generatedFields = ['id', 'creato', 'modificato'];
    // fixture da caricare
    $this->fixtures = '_entityTestFixtures';
    // SQL read
    $this->canRead = ['gs_allegato' => ['id', 'creato', 'modificato', 'titolo', 'nome', 'estensione', 'dimensione', 'file', 'comunicazione_id']];
    // SQL write
    $this->canWrite = ['gs_allegato' => ['id', 'creato', 'modificato', 'titolo', 'nome', 'estensione', 'dimensione', 'file', 'comunicazione_id']];
    // SQL exec
    ...
    for ($i = 0; $i < 5; $i++) {
      ...
      foreach ($this->fields as $field) {
        $data[$i][$field] =
          ...
          ($field == 'comunicazione' ? $this->getReference("documento_word_1") :
          null))))));
  ...
  public function testValidation() {
    ...
    // titolo
    $property = $this->getPrivateProperty(Allegato::class, 'titolo');
    ...
    // nome
    $property = $this->getPrivateProperty(Allegato::class, 'nome');
    $property->setValue($existent, '');
    $err = $this->val->validate($existent);
    ...
    // estensione
    $property = $this->getPrivateProperty(Allegato::class, 'estensione');
    $property->setValue($existent, '');
    $err = $this->val->validate($existent);
    ...
    $existent->setEstensione(str_repeat('*', 33));
    $err = $this->val->validate($existent);
    $this->assertTrue(count($err) == 1 && $err[0]->getMessageTemplate() == 'field.maxlength', $this->entity.'::Estensione - MAX LENGTH');
    $existent->setEstensione(str_repeat('*', 32));
    $this->assertCount(0, $this->val->validate($existent), $this->entity.'::Estensione - VALID MAX LENGTH');
    // dimensione
    $existent->setDimensione(-5);
    $err = $this->val->validate($existent);
    $this->assertTrue(count($err) == 1 && $err[0]->getMessageTemplate() == 'field.zeropositive', $this->entity.'::Dimensione - ZEROPOSITIVE');
    $existent->setDimensione(0);
    $this->assertCount(0, $this->val->validate($existent), $this->entity.'::Dimensione - VALID ZEROPOSITIVE');
    $existent->setDimensione(1);
    $this->assertCount(0, $this->val->validate($existent), $this->entity.'::Dimensione - VALID ZEROPOSITIVE');
    // file
    $property = $this->getPrivateProperty(Allegato::class, 'file');
    $property->setValue($existent, '');
    $err = $this->val->validate($existent);

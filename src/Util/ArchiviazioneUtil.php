...
class ArchiviazioneUtil {
    ...
    for ( ; $data <= $data_fine; $data->modify('+1 day')) {
      ...
      foreach ($annotazioni as $a) {
        ...
        if ($a->getAvviso() && ($a->getAvviso()->getGenitori() == 'U' ||
          ...
          // legge genitori destinatari
          $genitoriAnnotazione = $this->em->getRepository(Alunno::class)->createQueryBuilder('a')
        ...
        foreach ($genitoriAnnotazione as $alu) {
          $genitori[] = $alu->getCognome().' '.$alu->getNome();
        }
        $dati['annotazioni'][] = [
      ...
      if (count($dati['note']) > 0 || count($dati['annotazioni']) > 0) {
        ...
        if (count($dati['annotazioni']) > 0) {
          ...
          foreach ($dati['annotazioni'] as $an) {
            ...
            if (count($an['genitori']) > 0) {
              $html .= '<i>Destinatari genitori: <b>'.implode('</b>, <b>', $an['genitori']).'</b></i><br>';
            }
            if (count($an['alunni']) > 0) {
              $html .= '<i>Destinatari alunni: <b>'.implode('</b>, <b>', $an['alunni']).'</b></i><br>';
            }
   ...
   */
  public function registroClassePrivacy(Classe $classe, Alunno $alunno): void {
    // inizializza
    $fs = new Filesystem();
    // percorso destinazione
    $percorso = $this->root.'/registri/classi';
    ...
    foreach ($datiPeriodi as $periodo) {
      if (!empty($periodo['nome']) && $oggi >= $periodo['inizio']) {
        // registro per il periodo indicato
        $this->copertinaRegistroClassePrivacy($classe, $periodo);
        $this->scriveRegistroClassePrivacy($classe, $alunno, $periodo);
      }
    }
    // salva il documento
    $this->pdf->save($percorso.'/'.$nomefile);
    // registro creato
    $this->reqstack->getSession()->getFlashBag()->add('success', 'Registro di classe privacy '.$classe.
      ' archiviato.');
  }
   ...
   * @param array $periodo Informazioni sul periodo di riferimento
   */
  public function copertinaRegistroClassePrivacy(Classe $classe, array $periodo): void {
    // nuova pagina
    $this->pdf->getHandler()->AddPage('L');
    // crea copertina
    $this->pdf->getHandler()->SetFont('times', '', 14);
   ...
   * @param array $periodo Informazioni sul periodo di riferimento
   */
  public function scriveRegistroClassePrivacy(Classe $classe, Alunno $alunno, array $periodo): void {
    // inizializza dati
    $periodo_s = $periodo['nome'];
    $annoscolastico = $this->reqstack->getSession()->get('/CONFIG/SCUOLA/anno_scolastico').
      ' - '.$periodo_s;
    ...
    $fine = DateTime::createFromFormat('Y-m-d H:i', $periodo['fine'].' 00:00');
    $data_fine = $fine < $oggi ? $fine : $oggi;
    for ( ; $data <= $data_fine; $data->modify('+1 day')) {
      $dati['lezioni'] = [];
      $dati['note'] = [];
      $dati['annotazioni'] = [];
      ...
      foreach ($note as $n) {
        $alunni = [];
        foreach ($n->getAlunni() as $alu) {
          if ($alu->getId() == $alunno->getId()) {
            $alunni[] = $alu->getCognome().' '.$alu->getNome();
          }
        }
        if ($n->getTipo() == 'I' && count($alunni) == 0) {
          // nota di tipo individuale ma non riferita all'alunno che fa accesso agli atti: non la mostra
          continue;
        }
        $dati['note'][] = [
          'tipo' => $n->getTipo(),
      ...
      foreach ($annotazioni as $a) {
        ...
        if ($a->getAvviso() && ($a->getAvviso()->getAlunni() == 'U' ||
          ...
          $alunniAnnotazione = $this->em->getRepository(Alunno::class)->createQueryBuilder('a')
            ->join(ComunicazioneUtente::class, 'cu', 'WITH', 'cu.utente=a.id')
            ->where('a.id=:alunno AND cu.comunicazione=:avviso')
            ->setParameter('alunno', $alunno->getId())
            ->setParameter('avviso', $a->getAvviso())
            ->getQuery()
            ->getResult();
          if (count($alunniAnnotazione) == 0) {
            // annotazione con avviso per alunni ma non riferita all'alunno che fa accesso agli atti
            continue;
          }
        }
        foreach ($alunniAnnotazione as $alu) {
        ...
        if ($a->getAvviso() && ($a->getAvviso()->getGenitori() == 'U' ||
          ...
          $genitoriAnnotazione = $this->em->getRepository(Alunno::class)->createQueryBuilder('a')
            ...
            ->getQuery()
            ->getResult();
          if (count($genitoriAnnotazione) == 0) {
            // annotazione con avviso per genitori ma non riferita al genitore dell'alunno che fa accesso agli atti
            continue;
          }
        }
        foreach ($genitoriAnnotazione as $alu) {
   ...
   * @param Alunno $alunno Alunno che fa accesso agli atti (per omettere i dati degli altri alunni)
   */
  public function registroDocentePrivacy(Classe $classe, Alunno $alunno): void {
  // inizializza
    $fs = new Filesystem();
    // percorso destinazione
    $percorso = $this->root.'/registri/docenti';
    ...
    // nome documento
    $nomefile = 'registri-docenti-privacy-'.$classe->getAnno().$classe->getSezione().$classe->getGruppo().'-'.
      $alunno->getId().'.pdf';
    // lista cattedre (escluso sostegno e religione)
    $cattedre = $this->em->getRepository(Cattedra::class)->createQueryBuilder('c')
      ->join('c.docente', 'd')
      ->join('c.materia', 'm')
      ->join('c.classe', 'cl')
      ->where("cl.id=:classe AND m.tipo IN ('N', 'E')")
      ->orderBy('m.ordinamento,m.nome,d.cognome,d.nome')
      ->setParameter('classe', $classe->getId())
      ->getQuery()
      ->getResult();
    // aggiunge cattedre di religione
    if ($alunno->getReligione() == 'S' || $alunno->getReligione() == 'A') {
      // alunno con religione: aggiunge cattedra di religione/attività alternativa
      $cattedreReligione = $this->em->getRepository(Cattedra::class)->createQueryBuilder('c')
        ->join('c.docente', 'd')
        ->join('c.materia', 'm')
        ->join('c.classe', 'cl')
        ->where("cl.id=:classe AND m.tipo='R' AND c.tipo=:tipo")
        ->orderBy('m.ordinamento,m.nome,d.cognome,d.nome')
        ->setParameter('classe', $classe->getId())
        ->setParameter('tipo', $alunno->getReligione() == 'S' ? 'N' : 'A')
        ->getQuery()
        ->getResult();
      $cattedre = array_merge($cattedreReligione, $cattedre);
    }
    // aggiunge cattedre di sostegno
    if ($alunno->getBes() == 'H') {
      // alunno BES: aggiunge cattedre di sostegno
      $cattedreSostegno = $this->em->getRepository(Cattedra::class)->createQueryBuilder('c')
        ->join('c.docente', 'd')
        ->join('c.materia', 'm')
        ->join('c.classe', 'cl')
        ->where("cl.id=:classe AND m.tipo='S' AND (c.alunno=:alunno OR c.alunno IS NULL)")
        ->orderBy('m.ordinamento,m.nome,d.cognome,d.nome')
        ->setParameter('classe', $classe->getId())
        ->setParameter('alunno', $alunno->getId())
        ->getQuery()
        ->getResult();
      $cattedre = array_merge($cattedre, $cattedreSostegno);
    }
    // crea documento
    $this->pdf->configure($this->reqstack->getSession()->get('/CONFIG/ISTITUTO/intestazione'),
    ...
    foreach ($cattedre as $cat) {
      // inizializza
      $this->copertinaRegistroDocentePrivacy($cat);
      $pagina = $this->pdf->getHandler()->PageNo();
      foreach ($datiPeriodi as $periodo) {
        if (!empty($periodo['nome'])) {
          // registro per il periodo indicato
          if ($cat->getMateria()->getTipo() == 'S') {
            ...
            $this->scriveRegistroSostegnoPrivacy($cat, $periodo);
          } else {
            // cattedra curricolare
            $this->scriveRegistroDocentePrivacy($cat, $alunno, $periodo);
          }
        }
      }
      // controlla dati presenti
   ...
   * @param Cattedra $cattedra Cattedra del docente
   */
  public function copertinaRegistroDocentePrivacy(Cattedra $cattedra) {
    // nuova pagina
    $this->pdf->getHandler()->AddPage('L');
    // crea copertina
    $this->pdf->getHandler()->SetFont('times', '', 14);
  ...
  /**
   * Scrive le lezioni del docente, con omissioni per la privacy (per accesso agli atti)
   *
   * @param Cattedra $cattedra Cattedra del docente
   * @param Alunno $alunno Alunno che fa accesso agli atti (per omettere i dati degli altri alunni)
   * @param array $periodo Informazioni sul periodo di riferimento
   */
  public function scriveRegistroDocentePrivacy(Cattedra $cattedra, Alunno $alunno, array $periodo) {
    // inizializza dati
    $docente = $cattedra->getDocente();
    $docente_s = $docente->getNome().' '.$docente->getCognome();
    $classe_s = ''.$cattedra->getClasse();
    ...
    $votiNoLezione = $this->em->getRepository(Valutazione::class)->createQueryBuilder('v')
      ...
      ->join('v.lezione', 'l')
      ->join(Firma::class, 'f', 'WITH', 'l.id=f.lezione AND f.docente=:docente')
      ->where('v.alunno=:alunno AND v.materia=:materia AND v.docente=:docente AND l.classe=:classe AND l.materia!=:materia AND l.data BETWEEN :inizio AND :fine')
    ...
    if ($ore > 0 || $votiNoLezione > 0) {
      ...
      foreach ($lezioni as $l) {
        if (!$data_prec || $l['data'] != $data_prec) {
          ...
          // alunni in classe per data
          $lista = $this->regUtil->alunniInData($l['data'], $cattedra->getClasse());
          if (in_array($alunno->getId(), $lista, true)) {
            $dati['lezioni'][$mese][$giorno][$alunno->getId()]['classe'] = 1;
          }
        }
        // aggiorna durata lezioni
        $dati['lezioni'][$mese][$giorno]['durata'] += $l['durata'];
    ...
    $osservazioni = $this->em->getRepository(OsservazioneAlunno::class)->createQueryBuilder('o')
      ->select('o.data,o.testo,a.id AS alunno_id,a.cognome,a.nome,a.dataNascita')
      ->join('o.alunno', 'a')
      ->where('a.id=:alunno AND o.cattedra=:cattedra AND o.data BETWEEN :inizio AND :fine')
      ->orderBy('o.data,a.cognome,a.nome,a.dataNascita', 'ASC')
      ->setParameter('alunno', $alunno->getId())
			->setParameter('cattedra', $cattedra)
			->setParameter('inizio', $periodo['inizio'])
    ...
    if ($periodo['scrutinio'] == 'F' && !empty($dati['alunni'])) {
      ...
      $proposte = $this->em->getRepository(PropostaVoto::class)->createQueryBuilder('pv')
        ->select('(pv.alunno) AS idalunno,pv.unico,pv.debito,pv.periodo')
        ->where('pv.alunno=:alunno AND pv.docente=:docente AND pv.classe=:classe AND pv.materia=:materia AND pv.periodo IN (:periodi)')
        ->setParameter('alunno', $alunno->getId())
        ->setParameter('docente', $docente)
        ->setParameter('classe', $cattedra->getClasse())
        ->setParameter('materia', $cattedra->getMateria())
  ...
  /**
   * Scrive le lezioni del docente di sostegno, con omissioni per la privacy (per accesso agli atti)
   *
   * @param Cattedra $cattedra Cattedra del docente
   * @param array $periodo Informazioni sul periodo di riferimento
   */
  public function scriveRegistroSostegnoPrivacy(Cattedra $cattedra, array $periodo) {
    // inizializza dati
    $docente = $cattedra->getDocente();
    $docente_s = $docente->getNome().' '.$docente->getCognome();
    $classe_s = ''.$cattedra->getClasse();

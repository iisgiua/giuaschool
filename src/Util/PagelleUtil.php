...
class PagelleUtil {
  ...
  public function riepilogoVotiDati(Classe $classe, $periodo) {
    ...
    if ($periodo == 'P' || $periodo == 'S') {
      ...
      // scrutinio finale
      $dati['scrutinio'] = $this->em->getRepository(Scrutinio::class)->findOneBy(['classe' => $classe,
        'periodo' => $periodo, 'stato' => 'C']);
      $dati['classe'] = $classe;
      // alunni scrutinati
  ...
  public function riepilogoVoti(Classe $classe, $periodo) {
    ...
    if ($periodo == 'P' || $periodo == 'S') {
      ...
      if (!$fs->exists($percorso.'/'.$nomefile)) {
        ...
        $this->pdf->getHandler()->setHeaderData('', 0, $this->session->get('/CONFIG/ISTITUTO/intestazione')."     ***     RIEPILOGO VOTI ".$classe->getAnno().'ª '.$classe->getSezione(), '', array(0,0,0), array(255,255,255));
  ...
  public function alunnoInScrutinio(Classe $classe, $alunno, $periodo) {
    ...
    if ($periodo == 'P' || $periodo == 'S') {
      ...
      $estero = $this->em->getRepository(Alunno::class)->createQueryBuilder('a')

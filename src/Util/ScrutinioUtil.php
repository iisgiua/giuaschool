...
class ScrutinioUtil {
  ...
  /**
   * @var RegistroUtil $reg Funzioni di utilità per il registro
   */
  private $reg;
  ...
  public function quadroProposte(Docente $docente, Classe $classe, $periodo) {
    ...
    if ($periodo == 'P') {
      ...
      $alunni = $this->em->getRepository('App\Entity\Alunno')->alunniInData(new \DateTime(), $classe);
    }
    foreach ($alunni as $alu) {
  ...
  public function quadroVoti(Docente $docente, Classe $classe, $periodo) {
    ...
    // legge i voti
    $dati['voti'] = $this->em->getRepository('App\Entity\VotoScrutinio')->voti($classe, $periodo, $lista);
  ...
  public function elencoVoti(Docente $docente, Classe $classe, Materia $materia, $periodo) {
    ...
    // legge i voti
    $voti = $this->em->getRepository('App\Entity\VotoScrutinio')->voti($classe, $periodo,
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_S_6_C(Docente $docente, Request $request, Form $form,
                                       Classe $classe, Scrutinio $scrutinio) {
    // inizializza messaggi di errore
    $this->reqstack->getSession()->getFlashBag()->clear();
    // legge dati form
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_S_C_6(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    // controlla se è possibile riapertura
    if (!($docente instanceOf Staff) || $scrutinio->getSincronizzazione()) {
      // errore
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_S_5_6(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    // inizializza
    $errore = array();
    $this->reqstack->getSession()->getFlashBag()->clear();
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_S_6_5(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    // inizializza messaggi di errore
    $this->reqstack->getSession()->getFlashBag()->clear();
    // aggiorna stato

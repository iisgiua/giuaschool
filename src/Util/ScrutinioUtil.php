...
class ScrutinioUtil {
  ...
  public function elencoProposte(Docente $docente, Classe $classe, Materia $materia, $tipo, $periodo) {
    ...
    if ($materia->getTipo() == 'R') {
      ...
      $lista_alunni = $this->em->getRepository(Alunno::class)->createQueryBuilder('a')
        ->select('a.id')
  ...
  public function datiScrutinio(Docente $docente, Classe $classe, $periodo, $stato) {
    ...
    if ($periodo == 'P' || $periodo == 'S') {
      ...
      switch ($stato) {
        ...
        case '2':
          // esito
          $dati = $this->quadroVotiRinviati($docente, $classe, $periodo);
          break;
        case '3':
          // credito o competenze
          if ($classe->getAnno() == 2) {
  ...
  public function presenzeDocenti(Docente $docente, Classe $classe, $periodo) {
    ...
    if ($periodo == 'X') {
      ...
      foreach ($docenti as $kdoc=>$doc) {
        if (count($doc['cattedre']) == 1 &&
  ...
  /**
   * Restituisce la situazione dei voti dello scrutinio rinviato
   *
   * @param Docente $docente Docente che inserisce i dati dello scrutinio
   * @param Classe $classe Classe relativa alle proposte di voto
   * @param string $periodo Periodo relativo allo scrutinio
  ...
  public function passaggioStato_P_4_5(Docente $docente, Request $request, Form $form,
    ...
    if (empty($errori)) {
      ...
      $def = $this->em->getRepository(DefinizioneScrutinio::class)->findOneByPeriodo('P');
  ...
  public function riepilogo(Docente $docente, Classe $classe, $periodo) {
    ...
    $scrutinio = $this->em->getRepository(Scrutinio::class)->createQueryBuilder('s')
      ...
      ->getOneOrNullResult();
    // esiti
    if ($periodo == 'F' || $periodo == 'E' || $periodo == 'X') {
      $lista = $this->alunniInScrutinio($classe, $periodo);
      $esiti = $this->em->getRepository(Esito::class)->createQueryBuilder('e')
        ...
        if (in_array($mat['tipo'], ['N', 'E']) || in_array($alu['religione'], ['S', 'A'])) {
          ...
          $ore = $this->em->getRepository(AssenzaLezione::class)->createQueryBuilder('al')
            ...
            ->setParameters(['alunno' => $alunno, 'materia' => $materia,
              ...
              'fine' => $this->session->get('/CONFIG/SCUOLA/anno_fine'), 'classe' => $classe->getId()])
            ->getQuery()
            ->getSingleScalarResult();
          $ore = ($ore ? ((int) $ore) : 0);
          // controllo ore assenze mat.alt. firmata con altra materia
          if ($mat['tipo'] == 'R' && $alu['religione'] == 'A') {
            $ore2 = $this->em->getRepository(AssenzaLezione::class)->createQueryBuilder('al')
              ->select('SUM(al.ore)')
              ->join('al.lezione', 'l')
              ->leftJoin('App:CambioClasse', 'cc', 'WITH', 'cc.alunno=al.alunno AND l.data BETWEEN cc.inizio AND cc.fine')
  ...
  public function controlloAssenze(Docente $docente, Classe $classe, $periodo) {
    ...
    $scrutinio_F = $this->em->getRepository(Scrutinio::class)->findOneBy(['periodo' => 'F', 'classe' => $classe]);
    $scrutinio_S = $this->em->getRepository(Scrutinio::class)->findOneBy(['periodo' => 'S', 'classe' => $classe]);
    $scrutinio_P = $this->em->getRepository(Scrutinio::class)->findOneBy(['periodo' => 'P', 'classe' => $classe]);
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_E_N_1(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    $this->session->getFlashBag()->clear();
    // legge dati
    $dati = $this->riepilogoSospesi($docente, $classe, 'E');
    // alunni con voto in scrutinio
    $alunni_esistenti = $this->em->getRepository(VotoScrutinio::class)->alunni($scrutinio);
    // inserimento voti
    foreach ($dati['alunni'] as $alunno=>$alu) {
      $alunno_obj = $this->em->getRepository(Alunno::class)->find($alunno);
      foreach ($dati['materie'] as $materia=>$mat) {
        $materia_obj = $this->em->getRepository(Materia::class)->find($materia);
        // esclude alunni NA per religione
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_E_1_N(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    // controlla se docente fa parte di staff
    if (!($docente instanceOf Staff)) {
      // errore
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_E_1_2(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    // inizializza messaggi di errore
    $this->session->getFlashBag()->clear();
    // legge dati form
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_E_2_1(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    // inizializza messaggi di errore
    $this->session->getFlashBag()->clear();
    // aggiorna stato
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_E_2_3(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    // inizializza
    $this->session->getFlashBag()->clear();
    $errore = array();
    ...
    // alunni della classe
    $lista_id = $this->alunniInScrutinio($classe, 'E');
    foreach ($lista_id as $id) {
      // recupera alunno
      $alunno = $this->em->getRepository(Alunno::class)->find($id);
      $sesso = ($alunno->getSesso() == 'M' ? 'o' : 'a');
      $nome = $alunno->getCognome().' '.$alunno->getNome();
      // elenco voti dell'alunno
      $dati = $this->elencoVotiAlunnoSospeso($docente, $alunno, 'E');
      // controlla errori
      $no_voto = 0;
      $insuff_cont = 0;
      $insuff_religione = false;
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_E_3_2(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    // cancella medie
    $this->em->getConnection()
      ->prepare("UPDATE gs_esito SET media=NULL,credito=NULL,credito_precedente=NULL WHERE scrutinio_id=:scrutinio")
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_E_3_4(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    // inizializza
    $errore = array();
    $this->session->getFlashBag()->clear();
    ...
    if (empty($errore)) {
      // legge definizione scrutinio e verbale
      $def = $this->em->getRepository(DefinizioneScrutinio::class)->findOneByPeriodo('E');
      $scrutinio_dati = $scrutinio->getDati();
      foreach ($def->getStruttura() as $step=>$args) {
        if ($args[0] == 'Argomento') {
          // resetta validazione
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_E_4_3(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    // aggiorna stato
    $scrutinio->setStato('3');
    $this->em->flush();
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_E_4_C(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    // inizializza messaggi di errore
    $this->session->getFlashBag()->clear();
    // legge dati form
   ...
   * @return boolean Vero se passaggio di stato eseguito correttamente, falso altrimenti
   */
  public function passaggioStato_E_C_4(Docente $docente, Request $request, Form $form,
                                        Classe $classe, Scrutinio $scrutinio) {
    // controlla se è possibile riapertura
    if (!($docente instanceOf Staff) || $scrutinio->getSincronizzazione()) {
      // errore
    ...
    $fs = new Filesystem();
    $finder = new Finder();
    $percorso = $this->root.'/esami/'.$classe->getAnno().$classe->getSezione();
    $num = 0;
    while ($fs->exists($percorso.'/BACKUP.'.$num)) {
      $num++;
    }
  ...
  public function riepilogoRinviati(Docente $docente, Classe $classe, $periodo) {
    ...
    foreach ($scrutinio->getDato('voti') as $alu=>$voti) {
      foreach ($voti as $mat=>$voto) {
        ...
        if ($dati['materie'][$mat]['media']) {
          // esclude religione dalla media
          if (!isset($somma[$alu])) {
            $somma[$alu] = (($dati['materie'][$mat]['tipo'] == 'C' && $voto['unico'] == 4) ||
              ($dati['materie'][$mat]['tipo'] == 'E' && $voto['unico'] == 3)) ? 0 : $voto['unico'];
            $numero[$alu] = 1;
          } else {
            $somma[$alu] += (($dati['materie'][$mat]['tipo'] == 'C' && $voto['unico'] == 4) ||
              ($dati['materie'][$mat]['tipo'] == 'E' && $voto['unico'] == 3)) ? 0 : $voto['unico'];
    ...
    // calcola medie
    foreach ($somma as $alu=>$sm) {
      $dati['medie'][$alu] = number_format($sm / $numero[$alu], 2, ',', null);
    }
    // restituisce dati
    return $dati;
  }
        ...
        if ($mat['tipo'] != 'R' || in_array($alu['religione'], ['S', 'A'])) {
          ...
          if (array_key_exists($alunno, $alunni_esistenti) && in_array($materia, $alunni_esistenti[$alunno])) {
            ...
            $this->em->getConnection()
              ...
              ->execute(['scrutinio' => $scrutinio->getId(), 'alunno' => $alunno, 'materia' => $materia,
                'unico' => $dati['voti'][$alunno][$materia]['unico'],
                'debito' => ($dati['voti'][$alunno][$materia]['unico'] < 6 ? 'debito' : null),
                'recupero' => null,
                'assenze' => $dati['voti'][$alunno][$materia]['assenze'],
                'dati' => null]);
          }
  ...
  public function passaggioStato_X_2_3(Docente $docente, Request $request, Form $form,
    ...
    foreach ($lista_id as $id) {
      ...
      $nome = $alunno->getCognome().' '.$alunno->getNome();
      // elenco voti dell'alunno
      $dati = $this->elencoVotiAlunnoRinviato($docente, $alunno, $classe, 'X');
      // controlla errori
      $no_voto = 0;
      $insuff_cont = 0;
      $insuff_religione = false;
  ...
  public function passaggioStato_X_3_4(Docente $docente, Request $request, Form $form,
    ...
    if (empty($errore)) {
      // legge definizione scrutinio e verbale
      $def = $this->em->getRepository(DefinizioneScrutinio::class)->findOneByPeriodo('X');
      $scrutinio_dati = $scrutinio->getDati();
      foreach ($def->getStruttura() as $step=>$args) {
        if ($args[0] == 'Argomento') {
          // resetta validazione
  ...
  public function passaggioStato_X_C_4(Docente $docente, Request $request, Form $form,
    ...
    $fs = new Filesystem();
    $finder = new Finder();
    $percorso = $this->root.'/rinviati/'.$classe->getAnno().$classe->getSezione();
    $num = 0;
    while ($fs->exists($percorso.'/BACKUP.'.$num)) {
      $num++;
    }
  ...
  public function passaggioStato_S_4_5(Docente $docente, Request $request, Form $form,
    ...
    if (empty($errori)) {
      // legge definizione scrutinio e verbale
      $def = $this->em->getRepository(DefinizioneScrutinio::class)->findOneByPeriodo('S');
      $scrutinio_dati = $scrutinio->getDati();
      foreach ($def->getStruttura() as $step=>$args) {
        if ($args[0] == 'Argomento') {
          // resetta validazione
  ...
  public function passaggioStato_S_C_5(Docente $docente, Request $request, Form $form,
    ...
    $fs = new Filesystem();
    $finder = new Finder();
    $percorso = $this->root.'/secondo/'.$classe->getAnno().$classe->getSezione();
    $num = 0;
    while ($fs->exists($percorso.'/BACKUP.'.$num)) {
      $num++;
    }

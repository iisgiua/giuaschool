...
class FestivitaRepository extends EntityRepository {
  ...
  public function giornoSuccessivo(\DateTime $data, Sede $sede=null) {
    // fine anno
    $fine = $this->_em->getRepository('AppBundle:Configurazione')->findOneByParametro('anno_fine');
    ...
    while ($fine && $succ->format('Y-m-d') < $fine->getValore()) {
      ...
      $weekdays = $this->_em->getRepository('AppBundle:Configurazione')->findOneByParametro('giorni_festivi_istituto');
  ...
  public function giornoPrecedente(\DateTime $data, Sede $sede=null) {
    // inizio anno
    $inizio = $this->_em->getRepository('AppBundle:Configurazione')->findOneByParametro('anno_inizio');

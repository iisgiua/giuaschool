...
class CircolareRepository extends EntityRepository {
  ...
  public function listaNuove(Utente $utente) {
    // legge circolari
    $circolari = $this->createQueryBuilder('c')
      ->join('AppBundle:CircolareUtente', 'cu', 'WHERE', 'cu.circolare=c.id AND cu.utente=:utente')
  ...
  public function listaTutte($cerca, $pagina, $limite, Utente $utente) {
    // legge circolari
    $query = $this->createQueryBuilder('c')
      ->leftJoin('AppBundle:CircolareUtente', 'cu', 'WHERE', 'cu.circolare=c.id AND cu.utente=:utente')
 ...
 /**
  ...
  public function numeroCircolariClasse(Classe $classe) {
    ...
    $circolari = $this->createQueryBuilder('c')
      ->select('COUNT(c)')
      ->join('AppBundle:CircolareClasse', 'cc', 'WHERE', 'cc.circolare=c.id AND cc.classe=:classe')
  ...
  public function firma(Circolare $circolare, Utente $utente) {
    // dati destinatario
    $cu = $this->_em->getRepository('AppBundle:CircolareUtente')->findOneBy(['circolare' => $circolare,
      'utente' => $utente]);
  ...
  public function firmaClasse(Classe $classe, $id) {
    $firme = array();
    // query
    $circolari = $this->_em->getRepository('AppBundle:CircolareClasse')->createQueryBuilder('cc')
      ->join('cc.circolare', 'c')
  ...
  public function statistiche(Circolare $circolare) {
    ...
    if ($stat[0]['tot'] > $stat[0]['lette']) {
      ...
      $classi = $this->createQueryBuilder('c')
        ...
        ->join('AppBundle:CircolareClasse', 'cc', 'WHERE', 'cc.circolare=c.id')
        ->join('cc.classe', 'cl')
  ...
  public function notifica(Circolare $circolare) {
    // legge destinatari
    $destinatari = $this->_em->getRepository('AppBundle:CircolareUtente')->createQueryBuilder('cu')

...
class AgendaUtil {
  ...
  public function agendaEventi(Docente $docente, $mese) {
    ...
    $verifiche = $this->em->getRepository('AppBundle:Avviso')->createQueryBuilder('a')
      ...
      ->getQuery()
      ->getResult();
    $verifiche2 = $this->em->getRepository('AppBundle:Avviso')->createQueryBuilder('a')
      ->join('a.cattedra', 'c')
      ->join('AppBundle:Cattedra', 'c2', 'WHERE', 'c2.classe=c.classe AND c2.docente=:docente AND c2.alunno IS NOT NULL')
    ...
    // compiti
    $compiti = $this->em->getRepository('AppBundle:Avviso')->createQueryBuilder('a')
      ->where('a.docente=:docente AND a.tipo=:tipo AND MONTH(a.data)=:mese')
      ->setParameters(['docente' => $docente, 'tipo' => 'P', 'mese' => $mese->format('n')])
      ->getQuery()
      ->getResult();
    $compiti2 = $this->em->getRepository('AppBundle:Avviso')->createQueryBuilder('a')
      ->join('a.cattedra', 'c')
      ->join('AppBundle:Cattedra', 'c2', 'WHERE', 'c2.classe=c.classe AND c2.docente=:docente AND c2.alunno IS NOT NULL')
      ->leftJoin('AppBundle:AvvisoIndividuale', 'avi', 'WHERE', 'avi.avviso=a.id')
      ->where('a.docente!=:docente AND a.tipo=:tipo AND MONTH(a.data)=:mese AND c2.attiva=:attiva')
  ...
  public function modificaFiltriVerificheCompiti(Avviso $avviso, $avviso_destinatari, $filtro, $filtro_id) {
    ...
    if ($filtro == 'C') {
      ...
      if (!in_array($filtro_id[0], array_column($avviso_destinatari['classi'], 'classe'))) {
        // nuova classe
        $classe = $this->em->getRepository('AppBundle:Classe')->find($filtro_id[0]);
        if ($classe) {
          $ac = (new AvvisoClasse())
            ->setAvviso($avviso)
            ->setClasse($classe);
      ...
      foreach (array_diff($filtro_id, array_column($avviso_destinatari['utenti'], 'alunno')) as $a) {
        ...
        $lista = $this->em->getRepository('AppBundle:Genitore')->findBy(['alunno' => $a]);
    ...
    foreach (array_diff(array_column($avviso_destinatari['classi'], 'classe'), array($filtro_id[0])) as $c) {
      $ac = $this->em->getRepository('AppBundle:AvvisoClasse')->findOneBy(['avviso' => $avviso->getId(), 'classe' => $c]);
      if ($ac) {
        $this->em->remove($ac);
      }
    }
    ...
    foreach (array_diff(array_column($avviso_destinatari['utenti'], 'alunno'), $filtro_id) as $a) {
      ...
      $lista = $this->em->getRepository('AppBundle:AvvisoIndividuale')->findBy(['avviso' => $avviso->getId(), 'alunno' => $a]);
      foreach ($lista as $ai) {
        $this->em->remove($ai);
      }
    }
  ...
  public function eliminaFiltriVerificheCompiti(Avviso $avviso) {
    ...
    $modifiche['utenti'] = [];
    // classi
    $classi = $this->em->getRepository('AppBundle:AvvisoClasse')->findBy(['avviso' => $avviso->getId()]);
    foreach ($classi as $c) {
      $modifiche['classi'][] = $c->getClasse()->getId();
      $this->em->remove($c);
    }
    // utenti
    $utenti = $this->em->getRepository('AppBundle:AvvisoIndividuale')->findBy(['avviso' => $avviso->getId()]);
    foreach ($utenti as $u) {

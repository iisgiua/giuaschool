...
use AppBundle\Entity\AvvisoSede;
use AppBundle\Entity\AvvisoClasse;
use AppBundle\Entity\AvvisoIndividuale;
...
class BachecaUtil {
  ...
  public function filtriAvviso(Avviso $avviso) {
    $dati = array();
    // legge sedi
    $dati['sedi'] = $this->em->getRepository('AppBundle:AvvisoSede')->createQueryBuilder('avs')
      ...
      ->getArrayResult();
    // legge classi
    $dati['classi'] = $this->em->getRepository('AppBundle:AvvisoClasse')->createQueryBuilder('avc')
      ...
      ->getArrayResult();
    // legge genitori
    $dati['utenti'] = $this->em->getRepository('AppBundle:AvvisoIndividuale')->createQueryBuilder('avi')
  ...
  public function modificaFiltriAvviso(Avviso $avviso, $avviso_destinatari, $staff_filtro, $staff_sedi, $destinatari,
    ...
    $sedi = array();
    if ($staff_filtro == 'T') {
      $sedi = $this->em->getRepository('AppBundle:Sede')->createQueryBuilder('s')
        ->getQuery()
        ->getResult();
    ...
    if ($avviso->getDestinatariIndividuali()) {
      $genitori = $this->em->getRepository('AppBundle:Genitore')->createQueryBuilder('g')
    ...
    foreach ($avviso_destinatari['sedi'] as $s) {
      $as = $this->em->getRepository('AppBundle:AvvisoSede')->find($s['id']);
      if ($as) {
    ...
    foreach ($avviso_destinatari['classi'] as $c) {
      $ac = $this->em->getRepository('AppBundle:AvvisoClasse')->find($c['id']);
      if ($ac) {
    ...
    foreach ($avviso_destinatari['utenti'] as $u) {
      $ai = $this->em->getRepository('AppBundle:AvvisoIndividuale')->find($u['id']);
      if ($ai) {
  ...
  public function listaAvvisi($ricerca, $pagina, $limite, Docente $docente, $tipo) {
    ...
    if (isset($ricerca['classe']) && $ricerca['classe']) {
      $avvisi = $avvisi
        ->join('AppBundle:AvvisoClasse', 'ac', 'WHERE', 'a.id=ac.avviso')
    ...
    if (isset($ricerca['classe_individuale']) && $ricerca['classe_individuale']) {
      $avvisi = $avvisi
        ->join('AppBundle:AvvisoIndividuale', 'ai', 'WHERE', 'a.id=ai.avviso')
        ->join('ai.alunno', 'al')
    ...
    foreach ($dati['lista'] as $k=>$a) {
      ...
      if ($tipo == 'C') {
        // legge sedi di staff
        $dati['sedi'][$a->getId()] = $this->em->getRepository('AppBundle:AvvisoSede')->createQueryBuilder('avs')
          ->select('s.citta')
          ->join('avs.sede', 's')
          ->where('avs.avviso=:avviso')
          ->orderBy('s.principale', 'DESC')
        ...
        $dati['classe_individuale'][$a->getId()] = $this->em->getRepository('AppBundle:AvvisoIndividuale')->createQueryBuilder('avi')
        ...
        // legge classi
        $dati['classi'][$a->getId()] = $this->em->getRepository('AppBundle:AvvisoClasse')->createQueryBuilder('avc')
  ...
  public function destinatario(Avviso $avviso, Utente $utente, &$letto) {
    ...
    if (($utente instanceOf Genitore) && $avviso->getDestinatariGenitori()) {
      ...
      if ($avviso->getDestinatariIndividuali()) {
        // filtro utente
        $avi = $this->em->getRepository('AppBundle:AvvisoIndividuale')->createQueryBuilder('avi')
          ->join('avi.genitore', 'g')
          ->join('avi.alunno', 'a')
      ...
      if ($avviso->getDestinatariDocenti()) {
        ...
        $avc = $this->em->getRepository('AppBundle:AvvisoClasse')->createQueryBuilder('avc')
          ->join('avc.classe', 'cl')
          ->join('AppBundle:Cattedra', 'c', 'WHERE', 'cl.id=c.classe')
      ...
      if (($utente instanceOf Staff) && $avviso->getDestinatariStaff()) {
        ...
        $avs = $this->em->getRepository('AppBundle:AvvisoSede')->createQueryBuilder('avs')
          ->join('avs.sede', 's')
          ->join('AppBundle:Staff', 'st', 'WHERE', 's.id=st.sede')
  ...
  public function eliminaFiltriAvviso(Avviso $avviso) {
    ...
    $modifiche['utenti'] = [];
    // sedi
    $sedi = $this->em->getRepository('AppBundle:AvvisoSede')->createQueryBuilder('avs')
      ->join('avs.sede', 's')
      ->where('avs.avviso=:avviso')
      ->setParameters(['avviso' => $avviso->getId()])
      ->orderBy('s.principale', 'DESC')
    ...
    // classi
    $classi = $this->em->getRepository('AppBundle:AvvisoClasse')->createQueryBuilder('avc')
      ->join('avc.classe', 'c')
      ->where('avc.avviso=:avviso')
      ->setParameters(['avviso' => $avviso->getId()])
      ->orderBy('c.anno,c.sezione', 'ASC')
    ...
    // utenti
    $utenti = $this->em->getRepository('AppBundle:AvvisoIndividuale')->createQueryBuilder('avi')
      ->join('avi.alunno', 'a')
      ->where('avi.avviso=:avviso')
      ->setParameters(['avviso' => $avviso->getId()])
      ->orderBy('a.cognome,a.nome', 'ASC')
  ...
  public function listaAvvisiCoordinatore($pagina, $limite, Docente $docente, Classe $classe) {
    ...
    // legge avvisi
    $avvisi = $this->em->getRepository('AppBundle:Avviso')->createQueryBuilder('a')
      ->leftJoin('AppBundle:AvvisoClasse', 'ac', 'WHERE', 'a.id=ac.avviso')
      ->leftJoin('AppBundle:AvvisoIndividuale', 'ai', 'WHERE', 'a.id=ai.avviso')
    ...
    foreach ($dati['lista'] as $k=>$a) {
      ...
      if ($a->getDestinatariIndividuali()) {
        ...
        $dati['utenti'][$a->getId()] = $this->em->getRepository('AppBundle:AvvisoIndividuale')->createQueryBuilder('avi')

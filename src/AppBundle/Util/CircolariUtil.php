...
class CircolariUtil {
  ...
  public function listaCircolari($ricerca, $pagina, $limite, Staff $docente) {
    ...
    $dati['bozza'] = $this->em->getRepository('AppBundle:Circolare')->bozza();
    ...
    $dati['lista'] = $this->em->getRepository('AppBundle:Circolare')->pubblicate($ricerca, $pagina, $limite);
    // controllo azioni e aggiunta info
  ...
  public function destinatari(Circolare $circolare) {
    ...
    if ($circolare->getDsga()) {
      // aggiunge DSGA
      $utenti = $this->em->getRepository('AppBundle:Ata')->getIdDsga();
    }
    // ata
    if ($circolare->getAta()) {
      // aggiunge ATA
      $utenti = array_merge($utenti, $this->em->getRepository('AppBundle:Ata')->getIdAta($sedi));
    }
    // coordinatori
    if ($circolare->getCoordinatori() != 'N') {
      // aggiunge coordinatori
      $utenti = array_merge($utenti, $this->em->getRepository('AppBundle:Docente')
    ...
    if ($circolare->getDocenti() != 'N') {
      ...
      if ($circolare->getDocenti() == 'U') {
        ...
        $utenti = array_merge($utenti, $this->em->getRepository('AppBundle:Staff')
    ...
    if ($circolare->getGenitori() != 'N') {
      // aggiunge genitori
      $utenti = array_merge($utenti, $this->em->getRepository('AppBundle:Genitore')
        ->getIdGenitore($sedi, $circolare->getGenitori(), $circolare->getFiltroGenitori()));
      if ($circolare->getGenitori() != 'U') {
        // aggiunge classi
        $classi = array_merge($classi, $this->em->getRepository('AppBundle:Classe')
    ...
    if ($circolare->getAlunni() != 'N') {
      // aggiunge alunni
      $utenti = array_merge($utenti, $this->em->getRepository('AppBundle:Alunno')
 ...
 /**
  ...
  public function dettagli(Circolare $circolare) {
    ...
    if ($circolare->getCoordinatori() == 'C') {
      $dati['coordinatori'] = $this->em->getRepository('AppBundle:Classe')->listaClassi($circolare->getFiltroCoordinatori());
    }
    // docenti
    if ($circolare->getDocenti() == 'C') {
      $dati['docenti'] = $this->em->getRepository('AppBundle:Classe')->listaClassi($circolare->getFiltroDocenti());
    } elseif ($circolare->getDocenti() == 'M') {
      $dati['docenti'] = $this->em->getRepository('AppBundle:Materia')->listaMaterie($circolare->getFiltroDocenti());
    } elseif ($circolare->getDocenti() == 'U') {
      $dati['docenti'] = $this->em->getRepository('AppBundle:Docente')->listaDocenti($circolare->getFiltroDocenti(), 'gs-docenti-');
    }
    // genitori
    if ($circolare->getGenitori() == 'C') {
      $dati['genitori'] = $this->em->getRepository('AppBundle:Classe')->listaClassi($circolare->getFiltroGenitori());
    } elseif ($circolare->getGenitori() == 'U') {
      $dati['genitori'] = $this->em->getRepository('AppBundle:Alunno')->listaAlunni($circolare->getFiltroGenitori(), 'gs-genitori-');
    }
    // alunni
    if ($circolare->getAlunni() == 'C') {
      $dati['alunni'] = $this->em->getRepository('AppBundle:Classe')->listaClassi($circolare->getFiltroAlunni());
    } elseif ($circolare->getAlunni() == 'U') {
      $dati['alunni'] = $this->em->getRepository('AppBundle:Alunno')->listaAlunni($circolare->getFiltroAlunni(), 'gs-alunni-');
    }
    // statistiche di lettura
    if ($circolare->getPubblicata()) {
      $dati['lettura'] = $this->em->getRepository('AppBundle:Circolare')->statistiche($circolare);
    }
    // restituisce dati
    return $dati;
  }
  ...
  public function permessoLettura(Circolare $circolare, Utente $utente) {
    if (($utente instanceOf Docente) || ($utente instanceOf Ata)) {
      ...
      $cu = $this->em->getRepository('AppBundle:CircolareUtente')->findOneBy(['circolare' => $circolare, 'utente' => $utente]);
      return ($cu != null);
    }
    // non è autorizzato
    return false;

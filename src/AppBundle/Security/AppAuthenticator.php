...
class AppAuthenticator extends AbstractGuardAuthenticator {
  ...
  public function checkCredentials($credentials, UserInterface $user) {
    ...
    if ($this->encoder->isPasswordValid($user, $plainPassword)) {
      ...
      $app = $this->em->getRepository('AppBundle:App')->findOneBy(['token' => $credentials['appId'], 'attiva' => 1]);
      if (!$app) {
  ...
  public function onAuthenticationSuccess(Request $request, TokenInterface $token, $providerKey) {
    ...
    $testo = base64_decode(str_replace(array('-', '_'), array('+', '/'), $codice));
    $appId = substr($testo, $lusr + $lpsw, $lapp);
    $app = $this->em->getRepository('AppBundle:App')->findOneBy(['token' => $appId, 'attiva' => 1]);

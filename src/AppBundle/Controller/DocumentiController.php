...
class DocumentiController extends Controller {
  ...
  public function pianoEditAction(Request $request, EntityManagerInterface $em, SessionInterface $session,
    ...
    if ($id > 0) {
      // azione edit
      $documento = $em->getRepository('AppBundle:Documento')->findOneBy(['id' => $id, 'tipo' => 'L',
        'classe' => $classe, 'materia' => $materia]);
      if (!$documento) {
        // errore
        throw $this->createNotFoundException('exception.id_notfound');
      ...
      // azione add
      $documento = $em->getRepository('AppBundle:Documento')->findOneBy(['tipo' => 'L',
        'classe' => $classe, 'materia' => $materia]);
      if ($documento) {
        // errore
        throw $this->createNotFoundException('exception.id_notfound');
      ...
      ->add('classe', EntityType::class, array('label' => 'label.classe',
        ...
        'query_builder' => function (EntityRepository $er) use ($docente) {
            return $er->createQueryBuilder('c')
              ->join('AppBundle:Cattedra', 'ca', 'WHERE', 'ca.classe=c.id')
  ...
  public function doc15EditAction(Request $request, EntityManagerInterface $em, SessionInterface $session,
    ...
    $info = null;
    // controlla classe
    $classe = $em->getRepository('AppBundle:Classe')->findOneBy(['id' => $classe, 'coordinatore' => $this->getUser()]);
    if (!$classe) {
      // errore
      throw $this->createNotFoundException('exception.id_notfound');
    }
    ...
    if ($id > 0) {
      // azione edit
      $documento = $em->getRepository('AppBundle:Documento')->findOneBy(['id' => $id, 'tipo' => 'M',
        'classe' => $classe]);
      if (!$documento) {
        // errore
        throw $this->createNotFoundException('exception.id_notfound');
      ...
      // azione add
      $documento = $em->getRepository('AppBundle:Documento')->findOneBy(['tipo' => 'M',
        'classe' => $classe]);
      if ($documento) {
        // errore
        throw $this->createNotFoundException('exception.id_notfound');

...
class AtaController extends Controller {
  ...
  public function modificaAction(Request $request, EntityManagerInterface $em, SessionInterface $session, $pagina) {
    ...
    // lista
    $dati = $em->getRepository('AppBundle:ATA')->findAll($search, $pagina, $limite);
  ...
  public function abilitaAction(EntityManagerInterface $em, $id, $abilita) {
    // controlla ata
    $ata = $em->getRepository('AppBundle:Ata')->find($id);
    if (!$ata) {
      // errore
  ...
  public function passwordAction(Request $request, EntityManagerInterface $em, UserPasswordEncoderInterface $encoder,
    ...
    $message = (new \Swift_Message())
      ->setSubject('I.I.S. Credenziali di accesso al Registro Elettronico')

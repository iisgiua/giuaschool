...
class RegistroController extends Controller {
  ...
  public function addAction(Request $request, EntityManagerInterface $em, RegistroUtil $reg, LogHandler $dblogger,
    ...
    $lezione = $em->getRepository('AppBundle:Lezione')->findOneBy(['classe' => $classe, 'data' => $data_obj,
      'ora' => $ora]);
    if ($lezione) {
  ...
  public function editAction(Request $request, EntityManagerInterface $em, RegistroUtil $reg, LogHandler $dblogger,
    ...
    $firme = $em->getRepository('AppBundle:Firma')->findByLezione($lezione);
    if (count($firme) == 0) {
  ...
  public function deleteAction(Request $request, EntityManagerInterface $em, RegistroUtil $reg, LogHandler $dblogger,
    ...
    // controlla voti
    $voti = $em->getRepository('AppBundle:Valutazione')->findBy(['lezione' => $lezione, 'docente' => $this->getUser()]);
    if (count($voti) > 0) {
      // altra lezione
      $altra_lezione = $em->getRepository('AppBundle:Lezione')->createQueryBuilder('l')
    ...
    if (count($lista_firme) == 1) {
      ...
      $assenze_lezione = $em->getRepository('AppBundle:AssenzaLezione')->findByLezione($lezione);
      ...
      $lezione_cancellata['argomento'] = $lezione->getArgomento();
      $lezione_cancellata['attivita'] = $lezione->getAttivita();
      $materia = $em->getRepository('AppBundle:Materia')->findOneByTipo('S');
      if (!$materia) {
        // errore: dati inconsistenti
        throw $this->createNotFoundException('exception.invalid_params');
      }
  ...
  public function annotazioneEditAction(Request $request, EntityManagerInterface $em, RegistroUtil $reg, BachecaUtil $bac,
    ...
    if ($id > 0) {
      ...
      $annotazione = $em->getRepository('AppBundle:Annotazione')->findOneBy(['id' => $id,
  ...
  public function notaEditAction(Request $request, EntityManagerInterface $em, RegistroUtil $reg, LogHandler $dblogger,
    ...
    if ($id > 0) {
      ...
      $nota = $em->getRepository('AppBundle:Nota')->findOneBy(['id' => $id,
        'data' => $data_obj, 'classe' => $classe]);
      if (!$nota) {
        // errore
        throw $this->createNotFoundException('exception.id_notfound');

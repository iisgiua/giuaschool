...
use AppBundle\Entity\CircolareUtente;
use AppBundle\Entity\CircolareClasse;
use AppBundle\Entity\Notifica;
use AppBundle\Form\CircolareType;
use AppBundle\Util\RegistroUtil;
use AppBundle\Util\CircolariUtil;
 ...
 */
class CircolariController extends Controller {
  ...
  public function editAction(Request $request, EntityManagerInterface $em, SessionInterface $session,
    ...
    if ($id > 0) {
      // azione edit
      $circolare = $em->getRepository('AppBundle:Circolare')->findOneBy(['id' => $id]);
      if (!$circolare) {
        // errore
      ...
      // azione add
      $numero = $em->getRepository('AppBundle:Circolare')->prossimoNumero();
    ...
    $dati['coordinatori'] = ($form->get('coordinatori')->getData() == 'C' ?
      $em->getRepository('AppBundle:Classe')->listaClassi($form->get('filtroCoordinatori')->getData()) : '');
    $dati['docenti'] = ($form->get('docenti')->getData() == 'C' ?
      $em->getRepository('AppBundle:Classe')->listaClassi($form->get('filtroDocenti')->getData()) :
        ($form->get('docenti')->getData() == 'M' ?
        $em->getRepository('AppBundle:Materia')->listaMaterie($form->get('filtroDocenti')->getData()) :
          ($form->get('docenti')->getData() == 'U' ?
          $em->getRepository('AppBundle:Docente')->listaDocenti($form->get('filtroDocenti')->getData(), 'gs-filtroDocenti-') :'')));
    $dati['genitori'] = ($form->get('genitori')->getData() == 'C' ?
      $em->getRepository('AppBundle:Classe')->listaClassi($form->get('filtroGenitori')->getData()) :
        ($form->get('genitori')->getData() == 'U' ?
        $em->getRepository('AppBundle:Alunno')->listaAlunni($form->get('filtroGenitori')->getData(), 'gs-filtroGenitori-') :''));
    $dati['alunni'] = ($form->get('alunni')->getData() == 'C' ?
      $em->getRepository('AppBundle:Classe')->listaClassi($form->get('filtroAlunni')->getData()) :
        ($form->get('alunni')->getData() == 'U' ?
        $em->getRepository('AppBundle:Alunno')->listaAlunni($form->get('filtroAlunni')->getData(), 'gs-filtroAlunni-') :''));
    if ($form->isSubmitted()) {
      // lista sedi
      $sedi = array();
      ...
      if (!$em->getRepository('AppBundle:Circolare')->controllaNumero($circolare)) {
        // numero presente
      ...
      if ($circolare->getCoordinatori() == 'C') {
        // controlla classi
        $lista = $em->getRepository('AppBundle:Classe')
      ...
      if ($circolare->getDocenti() == 'C') {
        ...
        // controlla materie
        $lista = $em->getRepository('AppBundle:Materia')->controllaMaterie($form->get('filtroDocenti')->getData(), $errore);
        if ($errore) {
          // materia non valida
        ...
        // controlla utenti
        $lista = $em->getRepository('AppBundle:Docente')
      ...
      if ($circolare->getGenitori() == 'C') {
        ...
        // controlla utenti
        $lista = $em->getRepository('AppBundle:Alunno')
  ...
  public function deleteAction(Request $request, EntityManagerInterface $em, LogHandler $dblogger,
    ...
    $fs = new FileSystem();
    // controllo circolare
    $circolare = $em->getRepository('AppBundle:Circolare')->find($id);
    if (!$circolare) {
      // errore
      throw $this->createNotFoundException('exception.id_notfound');
    }
    ...
    if ($pubblica) {
      ...
      foreach ($dest['utenti'] as $u) {
        $obj = (new CircolareUtente())
          ...
          ->setUtente($em->getReference('AppBundle:Utente', $u));
        $em->persist($obj);
      }
      // imposta classi
      foreach ($dest['classi'] as $c) {
        $obj = (new CircolareClasse())
          ->setCircolare($circolare)
          ->setClasse($em->getReference('AppBundle:Classe', $c));
        $em->persist($obj);
      }
    } else {
      // rimuove destinatari
      $query = $em->getRepository('AppBundle:CircolareUtente')->createQueryBuilder('ce')
        ->delete()
        ->where('ce.circolare=:circolare')
        ->setParameters(['circolare' => $circolare])
        ->getQuery()
        ->execute();
      $query = $em->getRepository('AppBundle:CircolareClasse')->createQueryBuilder('cc')
        ->delete()
   ...
   *
   * @Route("/circolari/download/{id}/{doc}", name="circolari_download",
  ...
  public function downloadAction(EntityManagerInterface $em, CircolariUtil $circ, $id, $doc) {
    ...
    if ($doc == 0 && !$circolare->getFirma()) {
      ...
      $cu = $em->getRepository('AppBundle:CircolareUtente')->findOneBy(['circolare' => $circolare,
        'utente' => $this->getUser()]);
      if ($cu && !$cu->getLetta()) {
        // imposta lettura
  ...
  public function genitoriAction(EntityManagerInterface $em, SessionInterface $session, $pagina) {
    ...
    if ($pagina == 1) {
      // legge nuove circolari
      $dati['nuove'] = $em->getRepository('AppBundle:Circolare')->listaNuove($this->getUser());
    }
    // legge le altre circolari
    $dati['lista'] = $em->getRepository('AppBundle:Circolare')->listaLette($cerca, $pagina, $limite, $this->getUser());
    // mostra la pagina di risposta
    return $this->render('circolari/genitori.html.twig', array(
      'pagina_titolo' => 'page.circolari_genitori',
      'page' => $pagina,
      'maxPages' => $maxPages = ceil($dati['lista']->count() / $limite),
      'dati' => $dati,
    ));
  }
  ...
  public function dettagliDestinatariAction(EntityManagerInterface $em, CircolariUtil $circ, $id) {
    ...
    $dati = null;
    // controllo circolare
    $circolare = $em->getRepository('AppBundle:Circolare')->findOneBy(['id' => $id, 'pubblicata' => 1]);
    if (!$circolare || !$circ->permessoLettura($circolare, $this->getUser())) {
      // errore
      throw $this->createNotFoundException('exception.id_notfound');
    }
  ...
  public function firmaAction(EntityManagerInterface $em, CircolariUtil $circ, $id) {
    ...
    // firma
    $em->getRepository('AppBundle:Circolare')->firma($circolare, $this->getUser());
    // redirect
  ...
  public function docentiAction(Request $request, EntityManagerInterface $em, SessionInterface $session, $pagina) {
    ...
    // recupera criteri dalla sessione
    $cerca = array();
    $cerca['proprie'] = $session->get('/APP/ROUTE/circolari_docenti/proprie',
      ($this->getUser() instanceOf Staff ? false : true));
    $cerca['inizio'] = $session->get('/APP/ROUTE/circolari_docenti/inizio', null);
    $cerca['fine'] = $session->get('/APP/ROUTE/circolari_docenti/fine', null);
    $cerca['oggetto'] = $session->get('/APP/ROUTE/circolari_docenti/oggetto', '');
    if ($cerca['inizio']) {
      $inizio = \DateTime::createFromFormat('Y-m-d', $cerca['inizio']);
    } else {
      $inizio = \DateTime::createFromFormat('Y-m-d H:i', $session->get('/CONFIG/SCUOLA/anno_inizio').' 00:00')
        ->modify('first day of this month');
      $cerca['inizio'] = $inizio->format('Y-m-d');
    }
    if ($cerca['fine']) {
      $fine = \DateTime::createFromFormat('Y-m-d', $cerca['fine']);
    } else {
      $fine = new \DateTime('today');
      $cerca['fine'] = $fine->format('Y-m-d');
    }
    if ($pagina == 0) {
      // pagina non definita: la cerca in sessione
    ...
    $form = $this->container->get('form.factory')->createNamedBuilder('circolari_docenti', FormType::class)
      ->add('proprie', ChoiceType::class, array('label' => 'label.circolari_filtro_destinatario',
        'data' => $cerca['proprie'],
        'choices' => ['label.circolari_proprie' => true, 'label.circolari_tutte' => false],
        'expanded' => false,
        'multiple' => false,
        'label_attr' => ['class' => 'sr-only'],
        'required' => true))
    ...
    if ($form->isSubmitted() && $form->isValid()) {
      // imposta criteri di ricerca
      $cerca['proprie'] = $form->get('proprie')->getData();
      $cerca['inizio'] = ($form->get('inizio')->getData() ? $form->get('inizio')->getData()->format('Y-m-d') : 0);
      $cerca['fine'] = ($form->get('fine')->getData() ? $form->get('fine')->getData()->format('Y-m-d') : 0);
      $cerca['oggetto'] = $form->get('oggetto')->getData();
      $pagina = 1;
      $session->set('/APP/ROUTE/circolari_docenti/proprie', $cerca['proprie']);
      $session->set('/APP/ROUTE/circolari_docenti/inizio', $cerca['inizio']);
      $session->set('/APP/ROUTE/circolari_docenti/fine', $cerca['fine']);
    ...
    // legge le altre circolari
    if ($cerca['proprie']) {
      // recupera proprie circolari lette
      $dati['lista'] = $em->getRepository('AppBundle:Circolare')->listaLette($cerca, $pagina, $limite, $this->getUser());
    } else {
      // recupera tutte le circolari (lette o destinate ad altri)
      $dati['lista'] = $em->getRepository('AppBundle:Circolare')->listaTutte($cerca, $pagina, $limite, $this->getUser());
    }
    // mostra la pagina di risposta
  ...
  public function ataAction(Request $request, EntityManagerInterface $em, SessionInterface $session, $pagina) {
    ...
    // recupera criteri dalla sessione
    $cerca = array();
    $cerca['proprie'] = $session->get('/APP/ROUTE/circolari_ata/proprie', true);
    $cerca['inizio'] = $session->get('/APP/ROUTE/circolari_ata/inizio', null);
    $cerca['fine'] = $session->get('/APP/ROUTE/circolari_ata/fine', null);
    ...
    if ($form->isSubmitted() && $form->isValid()) {
      ...
      $cerca['oggetto'] = $form->get('oggetto')->getData();
      $pagina = 1;
      $session->set('/APP/ROUTE/circolari_ata/proprie', $cerca['proprie']);
      $session->set('/APP/ROUTE/circolari_ata/inizio', $cerca['inizio']);
      $session->set('/APP/ROUTE/circolari_ata/fine', $cerca['fine']);
  ...
  public function classiAction(EntityManagerInterface $em, $classe) {
    ...
    // legge dati
    $dati = $em->getRepository('AppBundle:Circolare')->circolariClasse($classe);
    // visualizza pagina
  ...
  public function firmaClasseAction(Request $request, EntityManagerInterface $em, LogHandler $dblogger,
    ...
    // firma
    $firme = $em->getRepository('AppBundle:Circolare')->firmaClasse($classe, $id);
    if (count($firme) > 0) {
      // lista circolari

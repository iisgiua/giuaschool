...
class LoginController extends Controller {
   ...
   *    methods={"GET", "POST"})
   */
  public function formAction(EntityManagerInterface $em, SessionInterface $session, ConfigLoader $config) {
    ...
    if ($form->isSubmitted() && $form->isValid()) {
      ...
      if (!$utente) {
        ...
        $message = (new \Swift_Message())
          ->setSubject('I.I.S. - Recupero credenziali del Registro Elettronico')
  ...
  public function attivazioneAction(Request $request, EntityManagerInterface $em, SessionInterface $session,
    ...
    if ($form->isSubmitted() && $form->isValid()) {
      ...
      $alunno = $em->getRepository('AppBundle:Alunno')->findOneByCodiceFiscale($codiceFiscale);
      if (!$alunno) {
        // utente non esiste
      ...
      } elseif ($em->getRepository('AppBundle:Utente')->findOneByEmail($email)) {
        // email esistente
        ...
        $message = (new \Swift_Message())
          ->setSubject('I.I.S. - Attivazione dell\'accesso al Registro Elettronico da parte degli studenti')
          ->setFrom(['@' => 'I.I.S. '])
          ->setTo([$email])
  ...
  public function confermaAction(Request $request, EntityManagerInterface $em, SessionInterface $session,
    ...
    // controlla token
    $ora = new \DateTime();
    $alunno = $em->getRepository('AppBundle:Alunno')->findOneBy(['token' => $token, 'abilitato' => 1]);
    if (!$alunno) {
      // utente inesistente
      ...
      // crea avviso
      $preside = $em->getRepository('AppBundle:Preside')->findOneBy(['abilitato' => 1]);
      $sesso = ($alunno->getSesso() == 'M' ? 'o' : 'a');
      ...
      $avviso = (new Avviso())
        ...
        ->setTesto($testo);
      $em->persist($avviso);
      $genitori = $em->getRepository('AppBundle:Genitore')->findByAlunno($alunno);

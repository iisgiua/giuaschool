...
class AgendaController extends Controller {
  ...
  public function verificaEditAction(Request $request, EntityManagerInterface $em, SessionInterface $session,
    ...
    if ($id > 0) {
      ...
      // controlla sostegno
      $sostegno = $em->getRepository('AppBundle:Cattedra')->findOneBy(['docente' => $docente,
      ...
      $mese = $em->getRepository('AppBundle:Festivita')->giornoSuccessivo($mese);
      $avviso = (new Avviso())
        ->setTipo('V')
    ...
    // form di inserimento
    $dati = $em->getRepository('AppBundle:Cattedra')->cattedreDocente($docente);
    ...
    if ($form->isSubmitted() && $form->isValid()) {
      ...
      if ($avviso->getCattedra() && $avviso->getCattedra()->getMateria()->getTipo() == 'S') {
        ...
        $materia = $em->getRepository('AppBundle:Cattedra')->findOneBy(['id' => $form->get('materia')->getData(),
  ...
  public function classeAjaxAction(EntityManagerInterface $em, $id) {
    ...
    $cattedre = $em->getRepository('AppBundle:Cattedra')->createQueryBuilder('c')
      ->select('c.id,m.nome')
      ->join('c.materia', 'm')
  ...
  public function compitoEditAction(Request $request, EntityManagerInterface $em, SessionInterface $session,
    ...
    if ($id > 0) {
      // azione edit
      $avviso = $em->getRepository('AppBundle:Avviso')->findOneBy(['id' => $id, 'tipo' => 'P']);
      if (!$avviso) {
        // errore
        throw $this->createNotFoundException('exception.id_notfound');
      }

 ...
 */
class ColloquiController extends Controller {
  ...
  public function colloquiAction(EntityManagerInterface $em, SessionInterface $session) {
    ...
    if ($oggi > $fine) {
      ...
      // legge richieste
      $dati['richieste'] = $em->getRepository('AppBundle:RichiestaColloquio')->colloquiDocente($this->getUser());
      $dati['ore'] = $em->getRepository('AppBundle:Colloquio')->ore($this->getUser());
    }
    // visualizza pagina
  ...
  public function colloquiEditAction(Request $request, EntityManagerInterface $em, RichiestaColloquio $richiesta, $azione) {
    ...
    $label = array();
    // controlla richiesta
    $richiesta = $em->getRepository('AppBundle:RichiestaColloquio')->find($richiesta);
    if (empty($richiesta)) {
      // errore

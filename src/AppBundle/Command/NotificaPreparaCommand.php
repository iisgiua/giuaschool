 ...
 */
class NotificaPreparaCommand extends ContainerAwareCommand {
  ...
  private function leggeNotifiche() {
    ...
    $notifiche = $this->em->getRepository('AppBundle:Notifica')->createQueryBuilder('n')
  ...
  private function creaMessaggi($dati) {
    ...
    $notifica_circolari = $this->em->getRepository('AppBundle:Configurazione')->parametro('notifica_circolari');
      ...
      if ($ogg != 'Circolare') {
        ...
        foreach ($destinatari as $id=>$circ) {
          ...
          if (empty($dati_notifica) && strpos('DT', $tipo) !== false) {
            ...
            $app = $this->em->getRepository('AppBundle:App')->findOneBy(['notifica' => 'E',
              'abilitati' => 'DT', 'attiva' => 1]);
          } elseif (!empty($dati_notifica)) {
            // legge app di notifica
            $app = $this->em->getRepository('AppBundle:App')->findOneBy(['id' => $dati_notifica['app'],
              'attiva' => 1]);
          }
  ...
  private function creaMessaggioAvviso($id, $notifica) {
    ...
    if ($notifica->getAzione() != 'D') {
      ...
      $avviso = $this->em->getRepository('AppBundle:Avviso')->find($id);
      if ($avviso) {
  ...
  private function creaMessaggioValutazione($id, $notifica) {
    ...
    if ($notifica->getAzione() != 'D') {
      ...
      $valutazione = $this->em->getRepository('AppBundle:Valutazione')->find($id);
  ...
  private function creaMessaggioUtente($id, $notifica) {
    ...
    if ($notifica->getAzione() != 'D') {
      ...
      $utente = $this->em->getRepository('AppBundle:Utente')->find($id);
  ...
  private function creaMessaggioCircolare($id, $notifica, &$destinatari) {
    if ($notifica->getAzione() != 'D') {
      ...
      $circolare = $this->em->getRepository('AppBundle:Circolare')->findOneBy(['id' => $id, 'pubblicata' => 1]);
      if ($circolare) {
        // solo circolari esistenti e pubblicate
        $utenti = $this->em->getRepository('AppBundle:Circolare')->notifica($circolare);
        foreach ($utenti as $u) {
          // memorizza circolari per utente
          $utente = $this->em->getRepository('AppBundle:Utente')->findOneBy(['id' => $u, 'abilitato' => 1]);
  ...
  private function creaNotificaInvio($testo, Utente $utente) {
    ...
    $tipo = ($utente instanceof Alunno ? 'A' : ($utente instanceof Genitore ? 'G' :
      ($utente instanceof Docente ? 'D' : ($utente instanceof Ata ? 'T' : ''))));
    $app = $this->em->getRepository('AppBundle:App')->findOneBy(['id' => $dati['app'], 'attiva' => 1]);
    if ($app && $app->getNotifica() != 'N' && $tipo && strpos($app->getAbilitati(), $tipo) !== false) {
      // crea notifica per l'invio
  ...
  private function cancellaMessaggi() {
    ...
    $num = $this->em->createQueryBuilder()
      ->delete('AppBundle:NotificaInvio', 'n')

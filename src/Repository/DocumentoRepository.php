...
class DocumentoRepository extends BaseRepository {
  ...
  public function piani(Docente $docente) {
    ...
    $cattedre = $this->_em->getRepository(\App\Entity\Cattedra::class)->createQueryBuilder('c')
      ...
      ->join('cl.corso', 'co')
      ->join('cl.sede', 's')
      ->leftJoin(\App\Entity\Documento::class, 'd', 'WITH', 'd.tipo=:documento AND d.classe=cl.id AND d.materia=m.id')
  ...
  public function relazioni(Docente $docente) {
    ...
    $cattedre = $this->_em->getRepository(\App\Entity\Cattedra::class)->createQueryBuilder('c')
      ...
      ->join('cl.sede', 's')
      ->leftJoin('c.alunno', 'a')
      ->leftJoin(\App\Entity\Documento::class, 'd', 'WITH', 'd.tipo=:documento AND d.classe=cl.id AND d.materia=m.id AND (m.tipo!=:sostegno OR (d.alunno=a.id AND d.docente=c.docente))')
  ...
  public function maggio(Docente $docente) {
    // query
    $cattedre = $this->_em->getRepository(\App\Entity\Classe::class)->createQueryBuilder('cl')
      ->select('cl.id AS classe_id,cl.anno,cl.sezione,cl.gruppo,co.nomeBreve AS corso,s.citta AS sede,d AS documento')
      ->join('cl.corso', 'co')
      ->join('cl.sede', 's')
      ->leftJoin(\App\Entity\Documento::class, 'd', 'WITH', 'd.tipo=:documento AND d.classe=cl.id')
  ...
  public function bes($criteri, $pagina, Sede $sede=null) {
    ...
    $alunni = $this->_em->getRepository(\App\Entity\Alunno::class)->createQueryBuilder('a')
      ->join(\App\Entity\Documento::class, 'd', 'WITH', 'd.alunno=a.id')
      ->join('a.classe', 'cl')
  ...
  public function lista($criteri, Utente $utente, $pagina) {
    ...
    $documenti = $this->createQueryBuilder('d')
      ...
      ->join(\App\Entity\ListaDestinatariUtente::class, 'ldu', 'WITH', 'ldu.listaDestinatari=ld.id')

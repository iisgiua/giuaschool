...
class DefinizioneConsultazioneRepository extends BaseRepository {
  ...
  public function lista(Utente $utente): array {
    ...
    $consultazioni = $this->createQueryBuilder('dc')
      ...
      ->where('dc.abilitata=1 AND :adesso BETWEEN dc.inizio AND dc.fine AND (dc.sede IS NULL OR dc.sede IN (:sedi)) AND (dc.classi IS NULL OR FIND_IN_SET(:classe, dc.classi) > 0)')
      ->andWhere($sql)

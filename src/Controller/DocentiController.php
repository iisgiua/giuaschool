...
class DocentiController extends BaseController {
  ...
  public function modificaAction(Request $request, EntityManagerInterface $em, SessionInterface $session, $page) {
    ...
    // lista docenti
    $paginator = $em->getRepository('App:Docente')->findAll($search, $page, $limit);
  ...
  public function cattedreAction(Request $request, EntityManagerInterface $em, SessionInterface $session, $page) {
    ...
    $classe = ($search['classe'] > 0 ? $em->getRepository('App:Classe')->find($search['classe']) : null);
    $materia = ($search['materia'] > 0 ? $em->getRepository('App:Materia')->find($search['materia']) : null);
    $docente = ($search['docente'] > 0 ? $em->getRepository('App:Docente')->find($search['docente']) : null);
    if ($page == 0) {
      // pagina non definita: la cerca in sessione
    ...
    // lista cattedre
    $paginator = $em->getRepository('App:Cattedra')->findAll($search, $page, $limit);
    // mostra la pagina di risposta
  ...
  public function staffAction(Request $request, EntityManagerInterface $em) {
    ...
    $form = $this->container->get('form.factory')->createNamedBuilder('docenti_staff', FormType::class)
      ->add('docente', EntityType::class, array('label' => 'label.docente',
        ...
        'query_builder' => function (EntityRepository $er) {
            return $er->createQueryBuilder('d')
              ->where('d INSTANCE OF App:Docente AND d.abilitato=1')
              ->orderBy('d.cognome,d.nome', 'ASC');
          },
        'required' => true))
    ...
    $staff = $em->getRepository('App:Staff')->findBy(array(), array('cognome' => 'ASC', 'nome' => 'ASC'));
    // mostra la pagina di risposta
  ...
  public function staffDeleteAction($id, EntityManagerInterface $em) {
    $user = $em->getRepository('App:Staff')->find($id);
    if ($user) {

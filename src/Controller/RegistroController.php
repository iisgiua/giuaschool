...
class RegistroController extends AbstractController {
  ...
  public function firmeAction(Request $request, EntityManagerInterface $em, SessionInterface $session,
    ...
    if ($cattedra > 0) {
      ...
      $materia = $em->getRepository(Materia::class)->findOneByTipo('U');
      if (!$materia) {
        // errore
    ...
    if ($classe) {
      ...
      if (!$errore) {
        ...
        if ($oggi->format('w') != 0 &&
            $adesso >= $em->getRepository(ScansioneOraria::class)->inizioLezioni($oggi, $classe->getSede()) &&
            $adesso <= $em->getRepository(ScansioneOraria::class)->fineLezioni($oggi, $classe->getSede())) {
          // avvisi alla classe
          $num_avvisi = $bac->bachecaNumeroAvvisiAlunni($classe);
          $lista_circolari = $em->getRepository(Circolare::class)->listaCircolariClasse($classe);
        }
        // recupera dati
        ...
        if ($vista == 'G') {
          ...
          if ($session->get('/CONFIG/SCUOLA/assenze_ore')) {
            ...
            $assenti = $em->getRepository(AssenzaLezione::class)->assentiOre($classe, $data_inizio);
          } else {
  ...
  public function addAction(Request $request, EntityManagerInterface $em, SessionInterface $session,
    ...
    $lezione = $em->getRepository(Lezione::class)->findOneBy(['classe' => $classe, 'data' => $data_obj,
      'ora' => $ora]);
    if ($lezione) {
    ...
    if ($session->get('/CONFIG/SCUOLA/assenze_ore')) {
      ...
      $assenti_precedenti = $em->getRepository(AssenzaLezione::class)->assentiLezionePrecedente($classe, $data_obj, $ora);
  ...
  public function editAction(Request $request, EntityManagerInterface $em, SessionInterface $session,
    ...
    $firme = $em->getRepository(Firma::class)->findByLezione($lezione);
    if (count($firme) == 0) {
    ...
    if ($cattedra) {
      // cattedra normale
      $altre_materie = $em->getRepository(Cattedra::class)->listaAltreMaterie($cattedra, $firme);
    } else {
      // supplenza
      $altre_materie = array();
    }
    ...
    if ($session->get('/CONFIG/SCUOLA/assenze_ore')) {
      // alunni assenti nell'ora
      $assenti_precedenti = $em->getRepository(AssenzaLezione::class)->assentiLezione($lezione);
      $religione = ($materia->getTipo() == 'R' && $cattedra->getTipo() == 'A') ? 'A' :
        ($materia->getTipo() == 'R' ? 'S' : '');
      $form = $form
        ->add('assenti', EntityType::class, array('label' => 'label.assenti',
    ...
    if ($form->isSubmitted() && $form->isValid()) {
      ...
      if (count($altre_materie) > 1) {
        // legge input
        $cattedra = $em->getRepository(Cattedra::class)->find($form->get('materia')->getData());
  ...
  public function deleteAction(Request $request, EntityManagerInterface $em, SessionInterface $session,
    ...
    // controlla voti
    $voti = $em->getRepository(Valutazione::class)->findBy(['lezione' => $lezione, 'docente' => $this->getUser()]);
    if (count($voti) > 0) {
      // altra lezione
      $altra_lezione = $em->getRepository(Lezione::class)->createQueryBuilder('l')
    ...
    if (count($lista_firme) == 1) {
      ...
      if ($session->get('/CONFIG/SCUOLA/assenze_ore')) {
        ...
        $assenze_lezione = $em->getRepository(AssenzaLezione::class)->findByLezione($lezione);
      ...
      $lezione_cancellata['argomento'] = $lezione->getArgomento();
      $lezione_cancellata['attivita'] = $lezione->getAttivita();
      $materia = $em->getRepository(Materia::class)->findOneByTipo('S');
      if (!$materia) {
        // errore: dati inconsistenti
        throw $this->createNotFoundException('exception.invalid_params');
      }
  ...
  public function annotazioneEditAction(Request $request, EntityManagerInterface $em, TranslatorInterface $trans,
    ...
    if ($id > 0) {
      ...
      $annotazione = $em->getRepository(Annotazione::class)->findOneBy(['id' => $id,
    ...
    if (!empty($dest_filtro)) {
      foreach ($dest_filtro as $id) {
        $alunni[] = $em->getRepository(Alunno::class)->find($id);
      }
    }
    // form di inserimento
  ...
  public function annotazioneDeleteAction(Request $request, EntityManagerInterface $em, RegistroUtil $reg,
    ...
    $annotazione = $em->getRepository(Annotazione::class)->find($id);
    if (!$annotazione) {
  ...
  public function notaEditAction(Request $request, EntityManagerInterface $em, TranslatorInterface $trans,
    ...
    if ($id > 0) {
      ...
      $nota = $em->getRepository(Nota::class)->findOneBy(['id' => $id,
        'data' => $data_obj, 'classe' => $classe]);
      if (!$nota) {
        // errore
        throw $this->createNotFoundException('exception.id_notfound');

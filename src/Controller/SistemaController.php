...
class SistemaController extends AbstractController {
  ...
  public function passwordAction(Request $request, EntityManagerInterface $em, UserPasswordEncoderInterface $encoder,
    ...
    // form
    $success = null;
    $form = $this->container->get('form.factory')->createNamedBuilder('sistema_password', FormType::class)
    ...
    return $this->render('sistema/password.html.twig', array(
  ...
  public function aliasAction(Request $request, EntityManagerInterface $em, SessionInterface $session,
    ...
    $form = $this->container->get('form.factory')->createNamedBuilder('sistema_alias', FormType::class)
      ->add('username', TextType::class, array('label' => 'label.username', 'required' => true))
      ->add('submit', SubmitType::class, array('label' => 'label.submit'))
      ->getForm();
    $form->handleRequest($request);
    ...
    // mostra la pagina di risposta
    return $this->render('sistema/alias.html.twig', array(
  ...
  //-- /**
   ...
   //-- * @return Response Pagina di risposta
   //-- *
   //-- * @Route("/sistema/ricalcola", name="sistema_ricalcola",
   //-- *    methods={"GET", "POST"})
   //-- *
   //-- * @IsGranted("ROLE_AMMINISTRATORE")
   //-- */
  //-- public function ricalcolaAction(Request $request, EntityManagerInterface $em, RegistroUtil $reg) {
    //-- // form
    //-- $success = null;
    //-- $form = $this->container->get('form.factory')->createNamedBuilder('sistema_ricalcola', FormType::class)
    ...
    //-- if ($form->isSubmitted() && $form->isValid()) {
      ...
      //-- $success = 'message.update_ok';
    //-- }
    //-- return $this->render('sistema/ricalcola.html.twig', array(
  ...
  public function manutenzioneAction(Request $request, EntityManagerInterface $em) {
    ...
    // form
    $form = $this->container->get('form.factory')->createNamedBuilder('sistema_manutenzione', FormType::class)
    ...
    $form = $form
      ->add('cancel', ButtonType::class, array('label' => 'label.cancel',
        ...
        //-- 'onclick' => "location.href='".$this->generateUrl('sistema')."'"
        ]))
      ->getForm();
    $form->handleRequest($request);
    if ($form->isSubmitted() && $form->isValid()) {
    ...
    // mostra la pagina di risposta
    return $this->render('sistema/manutenzione.html.twig', array(
   ...
   * @return Response Pagina di risposta
   *
   * @Route("/sistema/archiviazione/", name="sistema_archiviazione",
   *    methods={"GET", "POST"})
   *
   * @IsGranted("ROLE_AMMINISTRATORE")
   */
  public function archiviazioneAction(Request $request, EntityManagerInterface $em, TranslatorInterface $trans, ArchiviazioneUtil $arch) {
    ...
    $lista_classe = $em->getRepository('App:Classe')->createQueryBuilder('c')
      ...
      ->getResult();
    // form
    $form = $this->container->get('form.factory')->createNamedBuilder('sistema_archiviazione', FormType::class)
    ...
    // mostra la pagina di risposta
    return $this->render('sistema/archiviazione.html.twig', array(
